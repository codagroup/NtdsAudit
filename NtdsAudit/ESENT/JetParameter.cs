namespace CODA.NtdsAudit.ESENT;
/// <summary>
/// A mockup representation of <see href="https://learn.microsoft.com/en-gb/windows/win32/extensible-storage-engine/jet-param-enumeration"/> without the core esent.dll dependency
/// </summary>
public enum JetParameter
{
    SystemPath = 0,
    TempPath = 1,
    LogFilePath = 2,
    BaseName = 3,
    EventSource = 4,
    MaxSessions = 5,
    MaxOpenTables = 6,
    MaxCursors = 8,
    MaxVerPages = 9,
    MaxTemporaryTables = 10,
    LogFileSize = 11,
    LogBuffers = 12,
    CircularLog = 17,
    DbExtensionSize = 18,
    PageTempDBMin = 19,
    CacheSizeMax = 23,
    CheckpointDepthMax = 24,
    LrukCorrInterval = 25,
    LrukTimeout = 28,
    OutstandingIOMax = 30,
    StartFlushThreshold = 31,
    StopFlushThreshold = 32,
    Recovery = 34,
    EnableOnlineDefrag = 35,
    CacheSize = 41,
    EnableIndexChecking = 45,
    EventSourceKey = 49,
    NoInformationEvent = 50,
    EventLoggingLevel = 51,
    DeleteOutOfRangeLogs = 52,
    EnableIndexCleanup = 54,
    CacheSizeMin = 60,
    PreferredVerPages = 63,
    DatabasePageSize = 64,
    ErrorToString = 70,
    RuntimeCallback = 73,
    CleanupMismatchedLogFiles = 77,
    ExceptionAction = 98,
    CreatePathIfNotExist = 100,
    OneDatabasePerSession = 102,
    MaxInstances = 104,
    VersionStoreTaskQueueMax = 105,
    DisablePerfmon = 107
}
